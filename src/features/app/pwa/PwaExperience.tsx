...
export default function PwaExperience({ settings, home, onNotifications }: { settings: boolean; home: boolean; onNotifications: () => void }) {
  ...
  const [permission,setPermission] = useState("Niet beschikbaar");
  const [externalUpdate,setExternalUpdate] = useState(false);
  useEffect(() => {
    let active = true; const cleanups: (()=>void)[] = [];
    const network = () => setOnline(navigator.onLine);
    const install = () => { const state = installState(); setInstalled(installedContext() || state.installed); setAvailable(state.available); setIos(appleMobile()); setPermission("Notification" in window ? Notification.permission === "granted" ? "Toegestaan op dit apparaat" : Notification.permission === "denied" ? "Geweigerd op dit apparaat" : "Toestemming nog niet gevraagd" : "Niet beschikbaar"); };
  ...
  return <div class="pwa-experience">
    ...
    {settings && <section class="admin-panel pwa-settings" aria-labelledby="pwa-settings-title"><h2 id="pwa-settings-title">App</h2><p>Installatie: {installed ? "Geïnstalleerd / geopend als app" : "Geopend in de browser"}</p><p>Meldingen: {permission}. De verzendinstellingen staan hieronder.</p><p>Versie: <span data-app-version>{appBuild}</span></p>
