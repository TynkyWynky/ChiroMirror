...
export function validateReminders(rows: Reminder[], recurring = false) {
  if (rows.length > 10) throw new Error("Vous pouvez définir jusqu’à 10 rappels.");
  const signatures = new Set<string>();
  for (const row of rows) {
    if (row.kind === "LOCAL_TIME") {
      if (!Number.isInteger(row.days_before) || row.days_before! < 0 || row.days_before! > 365 || !row.local_time || row.offset_minutes !== null || row.scheduled_at !== null) throw new Error("Vérifiez les jours et l’heure du rappel.");
      Temporal.PlainTime.from(row.local_time);
    } else if (row.kind === "OFFSET") {
      if (!Number.isInteger(row.offset_minutes) || row.offset_minutes! < 0 || row.offset_minutes! > 525600 || row.days_before !== null || row.local_time !== null || row.scheduled_at !== null) throw new Error("Vérifiez le délai du rappel.");
    } else if (row.kind === "ABSOLUTE") {
      if (recurring) throw new Error("Utilisez des rappels relatifs pour une série récurrente.");
      if (!row.scheduled_at || row.offset_minutes !== null || row.days_before !== null || row.local_time !== null) throw new Error("Choisissez une date et une heure de rappel.");
      Temporal.Instant.from(row.scheduled_at);
    } else throw new Error("Rappel invalide.");
    ...
    if (signatures.has(signature)) throw new Error("Un même rappel est présent plusieurs fois.");
