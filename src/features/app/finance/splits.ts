...
export function equalSplit(total: bigint, entityIds: string[]): ExactShare[] {
  if (total <= 0n || total > MAX_CENTS || !entityIds.length || entityIds.length > 5000 || new Set(entityIds).size !== entityIds.length) throw new Error("Ongeldig bedrag of ongeldige deelnemers.");
...
export function customSplit(total: bigint, shares: ExactShare[]): ExactShare[] {
  if (total <= 0n || total > MAX_CENTS || !shares.length || shares.length > 5000 || new Set(shares.map(s => s.entity_id)).size !== shares.length
    || shares.some(s => s.amount_cents < 0n || s.amount_cents > MAX_CENTS) || shares.reduce((sum, s) => sum + s.amount_cents, 0n) !== total) throw new Error("De som van de aandelen moet exact gelijk zijn aan het totaal, zonder negatieve bedragen of dubbele deelnemers.");
  return shares;
}
