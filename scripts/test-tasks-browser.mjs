...
try {
  ...
  await db.query("insert into public.members(id,first_name,last_name,active) values($1,'Ancien','Lid',false)", [id(199)]);
  ...
  await page.getByRole("button", { name: "+ Teamtaak", exact: true }).click();
  await page.getByLabel("Titel *", { exact: true }).fill("Préparer le souper");
  await page.getByLabel("Status", { exact: true }).selectOption("IN_PROGRESS");
  await page.getByLabel("Prioriteit", { exact: true }).selectOption("HIGH");
  await page.getByLabel("Deadline", { exact: true }).selectOption("DATE");
  await page.getByLabel("Uiterste datum *", { exact: true }).fill("2026-09-22");
  assert.equal(await page.getByLabel("Rol van Ancien Lid").count(), 0);
  await page.getByRole("button", { name: "Opslaan", exact: true }).click();
  await page.getByRole("alert").filter({ hasText: "Kies minstens één verantwoordelijke" }).waitFor();
  await page.getByLabel("Rol van Camille Exemple").selectOption("LEAD");
  await page.getByLabel("Rol van Robin Exemple").selectOption("LEAD");
  await page.getByLabel("Rol van Alex Exemple").selectOption("CONTRIBUTOR");
  await page.getByRole("button", { name: "Opslaan", exact: true }).click();
  await page.getByText("Taak opgeslagen.", { exact: true }).waitFor();
  await page.getByRole("button", { name: "Préparer le souper", exact: true }).waitFor();
  await page.getByRole("button", { name: "Onder mijn toezicht", exact: true }).click();
  await page.getByText("0 / 3 personen klaar", { exact: true }).waitFor();
  ...
  assert.equal(await mobile.getByRole("button", { name: "+ Teamtaak", exact: true }).count(), 0);
  assert.equal(await mobile.getByRole("button", { name: "Alle", exact: true }).count(), 0);
  assert.equal(await mobile.evaluate(() => document.documentElement.scrollWidth <= innerWidth), true);
  await mobile.getByRole("button", { name: "✓ Mijn deel is klaar", exact: true }).click();
  await mobile.getByText("1 / 3 personen klaar", { exact: true }).waitFor();
  assert.equal((await db.query("select status from public.tasks where scope='TEAM'")).rows[0].status, "IN_PROGRESS");
  await mobile.getByRole("button", { name: "Mijn deel opnieuw openzetten", exact: true }).click();
  await mobile.getByText("0 / 3 personen klaar", { exact: true }).waitFor();
  await mobile.getByRole("button", { name: "✓ Mijn deel is klaar", exact: true }).click();
  await mobile.getByText("1 / 3 personen klaar", { exact: true }).waitFor();
  ...
  await mobile.getByRole("heading", { name: "Recente geschiedenis", exact: true }).waitFor();
  await mobile.locator(".task-audit").getByText("Deel afgerond", { exact: true }).first().waitFor();
  assert.equal(await mobile.getByRole("button", { name: /^(Bewerken|Taak afronden|Taak annuleren)$/ }).count(), 0);
  await mobile.getByRole("button", { name: "Details sluiten", exact: true }).click();
  await mobile.getByRole("button", { name: "+ Persoonlijke taak", exact: true }).click();
  assert.equal(await mobile.getByLabel(/Rol van/).count(), 0);
  await mobile.getByLabel("Titel *", { exact: true }).fill("Mon pense-bête privé");
  await mobile.getByRole("button", { name: "Opslaan", exact: true }).click();
  ...
  await page.getByLabel("Titel *", { exact: true }).fill("Titre périmé");
  await page.getByRole("button", { name: "Opslaan", exact: true }).click();
  await page.getByRole("alert").filter({ hasText: "elders gewijzigd" }).waitFor();
  await page.getByRole("button", { name: "Sluiten zonder op te slaan", exact: true }).click();
  await page.getByRole("button", { name: "Taken verversen", exact: true }).click();
  await page.getByText("1 / 3 personen klaar", { exact: true }).waitFor();
  await page.getByRole("button", { name: "Préparer le souper", exact: true }).click();
  let confirmation = "";
  page.once("dialog", async dialog => { confirmation = dialog.message(); await dialog.accept(); });
  await page.getByRole("button", { name: "Taak afronden", exact: true }).click();
  await page.getByText("Taak afgerond.", { exact: true }).waitFor();
  assert.match(confirmation, /2 personen/);
  ...
  await admin.getByRole("button", { name: "Alle", exact: true }).click();
  await admin.getByLabel("Filter op status", { exact: true }).selectOption("");
  ...
  await mobile.getByRole("button", { name: "Mijn taken bekijken", exact: true }).click();
  await mobile.getByRole("button", { name: "Mon pense-bête privé", exact: true }).waitFor();
