...
try {
  ...
  await page.getByRole("button", { name: "Maand", exact: true }).waitFor();
  await page.getByRole("button", { name: /Activiteit Activité du dimanche/ }).first().waitFor();
  const screenshots = `${root}/.test-artifacts`;
  mkdirSync(screenshots, { recursive: true });
  await page.screenshot({ path: `${screenshots}/agenda-desktop.png`, fullPage: true });
  await page.getByRole("button", { name: "+ Nieuwe activiteit", exact: true }).click();
  await page.getByLabel("Titel *", { exact: true }).fill("Événement navigateur");
  await page.getByLabel("Hele dag", { exact: true }).check();
  await page.getByLabel("Begin *", { exact: true }).fill("2026-09-24");
  await page.getByLabel("Einde *", { exact: true }).fill("2026-09-26");
  await page.getByLabel("Betrokken personen", { exact: true }).selectOption("SELECTED");
  assert.equal(await page.getByLabel("Alex Archivé").count(), 0);
  await page.getByLabel("Camille Exemple").check();
  await page.getByRole("button", { name: "Opslaan", exact: true }).click();
  await page.getByText("Activiteit opgeslagen.", { exact: true }).waitFor();
  await page.getByRole("button", { name: /Activiteit Événement navigateur/ }).first().click();
  ...
  await page.getByRole("button", { name: /Activiteit Activité du dimanche/ }).first().click();
  await page.getByRole("button", { name: "Dit exemplaar bewerken" }).click();
  await page.getByLabel("Titel *", { exact: true }).fill("Occurrence déplacée");
  await page.getByLabel("Begin *", { exact: true }).fill("2026-09-28T15:00");
  await page.getByLabel("Einde *", { exact: true }).fill("2026-09-28T17:00");
  await page.getByRole("button", { name: "Opslaan", exact: true }).click();
  await page.getByText("Activiteit opgeslagen.", { exact: true }).waitFor();
  await page.getByRole("button", { name: /Activiteit Occurrence déplacée/ }).first().click();
  page.once("dialog", dialog => dialog.accept());
  await page.getByRole("button", { name: "Dit exemplaar annuleren", exact: true }).click();
  await page.getByText("Annulering opgeslagen.", { exact: true }).waitFor();
  assert.equal(await page.getByRole("button", { name: /Occurrence déplacée/ }).count(), 0);
  await page.getByLabel("Geannuleerde activiteiten tonen").check();
  await page.getByRole("button", { name: /Geannuleerd Occurrence déplacée/ }).first().waitFor();
  await page.getByRole("button", { name: /Activiteit Activité du dimanche/ }).first().click();
  await page.getByLabel("Bereik van de actie", { exact: true }).selectOption("series");
  await page.getByRole("button", { name: "De hele reeks bewerken", exact: true }).click();
  await page.getByLabel("Titel *", { exact: true }).fill("Série navigateur");
  await page.getByRole("button", { name: "Opslaan", exact: true }).click();
  await page.getByText("Activiteit opgeslagen.", { exact: true }).waitFor();
  await page.getByRole("button", { name: /Activiteit Série navigateur/ }).first().waitFor();
  await page.getByRole("button", { name: /Geannuleerd Occurrence déplacée/ }).first().waitFor();
  ...
  await mobile.getByRole("button", { name: /Kamp Camp de démonstration/ }).waitFor();
  assert.equal(await mobile.getByRole("button", { name: "Lijst", exact: true }).getAttribute("aria-pressed"), "true");
  assert.equal(await mobile.getByRole("button", { name: /Nieuwe activiteit/ }).count(), 0);
  ...
  await mobile.getByRole("button", { name: /Kamp Camp de démonstration/ }).click();
  assert.equal(await mobile.getByRole("button", { name: /Bewerken|Annuleren/ }).count(), 0);
  assert.deepEqual(pageErrors, []);
  await mobile.goto(`http://127.0.0.1:${port}/?reader&summary`);
  await mobile.getByRole("heading", { name: "Komende activiteiten", exact: true }).waitFor();
  await mobile.locator("main li").first().waitFor();
  assert.ok(await mobile.locator("main li").count() <= 3);
  await mobile.getByRole("button", { name: "Agenda openen", exact: true }).click();
  await mobile.getByRole("button", { name: "Lijst", exact: true }).waitFor();
  assert.deepEqual(pageErrors, []);
