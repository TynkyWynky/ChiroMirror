...
try {
 ...
 await page.getByLabel('Naam van dit apparaat',{exact:true}).fill('Appareil partagé');await page.getByRole('button',{name:'Notificaties inschakelen',exact:true}).click();await page.getByText('Apparaat geregistreerd. Pushmeldingen zijn ingeschakeld.',{exact:true}).waitFor();
 ...
 await page.goto(base+scope+'?app=settings');await screen('Instellingen');await page.getByRole('button',{name:'Notificaties inschakelen',exact:true}).click();await page.getByText('Apparaat geregistreerd. Pushmeldingen zijn ingeschakeld.',{exact:true}).waitFor();
