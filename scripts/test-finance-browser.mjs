...
let server,browser;
try {
  for(const [n,first,role] of [[1,"Pierre","MEMBER"],[2,"Thomas","MEMBER"],[3,"Lucas","MEMBER"],[5,"Admin","APP_ADMIN"],[6,"Trésorier","TREASURER"]]) {
  ...
  await db.query("insert into public.members(id,first_name,last_name,active) values($1,'Arthur','Sans compte',true),($2,'Ancien','Membre',false)",[id(104),id(199)]);
  ...
  await page.getByRole("button",{name:"+ Nouvelle dette",exact:true}).click();
  await page.getByLabel("Motif *",{exact:true}).fill("Camp privé");
  await page.getByLabel("Montant total (€) *",{exact:true}).fill("80,00");
  await page.getByLabel("Qui doit ?",{exact:true}).selectOption(entity(2));
  await page.getByLabel("À qui ?",{exact:true}).selectOption(entity(1));
  assert.equal(await page.getByRole("option",{name:"Ancien Membre (archivé)"}).count(),0);
  ...
  await page.getByRole("button",{name:"Rembourser cette dette",exact:true}).click();
  await page.getByLabel("Montant du remboursement (€) *",{exact:true}).fill("80,01");
  await page.getByRole("button",{name:"Enregistrer le remboursement",exact:true}).click();
  await page.getByRole("alert").filter({hasText:"dépasse le solde restant"}).waitFor();
  await page.getByLabel("Montant du remboursement (€) *",{exact:true}).fill("30");
  await page.getByRole("button",{name:"Enregistrer le remboursement",exact:true}).click();
  await page.getByText("Remboursement enregistré.",{exact:true}).waitFor();
  ...
  assert.equal(await mobile.getByRole("button",{name:"Modifier",exact:true}).count(),0);
  await mobile.getByRole("button",{name:"Rembourser cette dette",exact:true}).click();
  await mobile.getByLabel("Montant du remboursement (€) *",{exact:true}).fill("10");
  await mobile.getByRole("button",{name:"Enregistrer le remboursement",exact:true}).click();
  await mobile.getByText("Remboursement enregistré.",{exact:true}).waitFor();
  await page.getByRole("button",{name:"Rembourser cette dette",exact:true}).click();
  await page.getByLabel("Montant du remboursement (€) *",{exact:true}).fill("40");
  await page.getByRole("button",{name:"Enregistrer le remboursement",exact:true}).click();
  await page.getByRole("alert").filter({hasText:"Cette opération a changé"}).waitFor();
  await page.getByRole("button",{name:"Fermer sans enregistrer",exact:true}).click();
  await page.getByRole("button",{name:"Actualiser les comptes",exact:true}).click();
  await page.getByTestId("receivable").filter({hasText:"40,00"}).waitFor();
  await mobile.getByRole("button",{name:"Rembourser cette dette",exact:true}).click();
  await mobile.getByRole("button",{name:"Tout le restant",exact:true}).click();
  await mobile.getByRole("button",{name:"Enregistrer le remboursement",exact:true}).click();
  await mobile.getByText("Remboursement enregistré.",{exact:true}).waitFor();
  await mobile.getByRole("button",{name:"Fermer la fiche",exact:true}).click();
  await mobile.getByRole("button",{name:"Historique",exact:true}).click();
  await mobile.getByLabel("Filtrer les opérations",{exact:true}).selectOption("SETTLED");
  ...
  await mobile.getByRole("button",{name:"Camp privé",exact:true}).click();
  await mobile.locator(".finance-obligation").filter({hasText:"40,00"}).getByRole("button",{name:"Annuler ce remboursement",exact:true}).click();
  await mobile.getByLabel("Motif d’annulation *",{exact:true}).fill("Correction du paiement saisi");
  await mobile.getByRole("button",{name:"Confirmer l’annulation",exact:true}).click();
  await mobile.getByText("Annulation enregistrée ; historique conservé.",{exact:true}).waitFor();
  await mobile.getByRole("button",{name:"Rembourser cette dette",exact:true}).click();
  await mobile.getByRole("button",{name:"Tout le restant",exact:true}).click();
  await mobile.getByRole("button",{name:"Enregistrer le remboursement",exact:true}).click();
  await mobile.getByText("Remboursement enregistré.",{exact:true}).waitFor();
  ...
  await page.getByRole("button",{name:"+ Nouvelle dépense",exact:true}).click();
  await page.getByLabel("Motif *",{exact:true}).fill("Souper partagé");
  await page.getByLabel("Montant total (€) *",{exact:true}).fill("147,82");
  ...
  await page.getByRole("heading",{name:"Parts de la dépense",exact:true}).waitFor();
  ...
  assert.equal(await page.getByRole("button",{name:"Rembourser cette dette",exact:true}).count(),3);
  ...
  await page.getByLabel("Motif d’annulation *",{exact:true}).fill("Exemple à remplacer");
  await page.getByRole("button",{name:"Confirmer l’annulation",exact:true}).click();
  await page.getByText("Annulation enregistrée ; historique conservé.",{exact:true}).waitFor();
  await page.getByRole("button",{name:"Fermer la fiche",exact:true}).click();
  await page.getByRole("button",{name:"+ Nouvelle dépense",exact:true}).click();
  await page.getByLabel("Motif *",{exact:true}).fill("Courses personnalisées");
  await page.getByLabel("Montant total (€) *",{exact:true}).fill("100");
  await page.getByLabel("Répartition",{exact:true}).selectOption("CUSTOM_AMOUNT");
  for(const [name,amount] of [["Pierre Exemple","20"],["Thomas Exemple","30"],["Lucas Exemple","40"]]) {await page.getByRole("checkbox",{name,exact:true}).check();await page.getByLabel(`Part de ${name} (€)`,{exact:true}).fill(amount);}
  await page.getByRole("button",{name:"Enregistrer",exact:true}).click();
  await page.getByRole("alert").filter({hasText:"somme des parts"}).waitFor();
  await page.getByLabel("Part de Lucas Exemple (€)",{exact:true}).fill("50");
  await page.getByRole("button",{name:"Enregistrer",exact:true}).click();
  await page.getByRole("button",{name:"Courses personnalisées",exact:true}).waitFor();
  assert.equal(await page.getByTestId("receivable").innerText(),formatMoney(8000n));

  const treasury=await pageFor(6);
  await treasury.getByRole("button",{name:"+ Nouvelle dette",exact:true}).click();
  await treasury.getByLabel("Visibilité",{exact:true}).selectOption("TREASURY");
  await treasury.getByLabel("Motif *",{exact:true}).fill("Solde camp Chiro");
  await treasury.getByLabel("Montant total (€) *",{exact:true}).fill("80");
  await treasury.getByLabel("Qui doit ?",{exact:true}).selectOption(entity(2));
  await treasury.getByLabel("À qui ?",{exact:true}).selectOption(chiro);
  await treasury.getByRole("button",{name:"Enregistrer",exact:true}).click();
  await treasury.getByText("Opération enregistrée.",{exact:true}).waitFor();
  await treasury.getByRole("button",{name:"Trésorerie",exact:true}).click();
  await treasury.getByRole("button",{name:"Solde camp Chiro",exact:true}).click();
  await treasury.getByRole("button",{name:"Rembourser cette dette",exact:true}).click();
  await treasury.getByLabel("Montant du remboursement (€) *",{exact:true}).fill("30");
  await treasury.getByRole("button",{name:"Enregistrer le remboursement",exact:true}).click();
  await treasury.getByText("Remboursement enregistré.",{exact:true}).waitFor();
  await mobile.goto(`${base}/?actor=2`);
  await mobile.getByRole("button",{name:"Solde camp Chiro",exact:true}).click();
  await mobile.getByRole("heading",{name:"Obligations",exact:true}).waitFor();
  assert.equal(await mobile.getByRole("button",{name:/Rembourser cette dette|Annuler ce remboursement|Modifier/}).count(),0);
  assert.match(await mobile.locator(".finance-detail").innerText(),/50,00/);
  const admin=await pageFor(5);
  assert.equal(await admin.getByRole("button",{name:"Trésorerie",exact:true}).count(),0);
  ...
  await page.getByText(/À recevoir :/).waitFor();
  assert.match(await page.locator("main").innerText(),/80,00/);
  await page.getByRole("button",{name:"Ouvrir mes comptes",exact:true}).click();
  await page.getByRole("button",{name:"Courses personnalisées",exact:true}).waitFor();
  assert.deepEqual(errors,[]);
  console.log("Comptes browser checks passed: desktop/mobile, private debt, partial/full payments, stale revision, equal/custom expense, cancellation, balances/history, treasury permissions, admin privacy and dashboard. Screenshots: .test-artifacts/");
