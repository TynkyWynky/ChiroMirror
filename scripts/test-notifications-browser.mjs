...
try{
 ...
 await page.getByRole('button',{name:'Notificaties inschakelen',exact:true}).waitFor();assert.equal(await page.evaluate(()=>window.pushFixture.requests),0);
 ...
 await page.getByRole('button',{name:'Notificaties inschakelen',exact:true}).click();await page.getByText('Apparaat geregistreerd. Pushmeldingen zijn ingeschakeld.',{exact:true}).waitFor();assert.equal(await page.evaluate(()=>window.pushFixture.requests),1);
 await page.getByRole('button',{name:'Testnotificatie',exact:true}).click();await page.getByText(/Test ingepland/).waitFor();assert.equal((await db.query("select count(*)::int n from public.notification_jobs where source_type='SYSTEM'")).rows[0].n,1);
 await page.getByRole('button',{name:'Uitschakelen op dit apparaat',exact:true}).click();await page.getByText('Notificaties uitgeschakeld op dit apparaat.',{exact:true}).waitFor();assert.equal((await db.query('select active v from public.push_subscriptions')).rows[0].v,false);
 ...
 await page.evaluate(()=>{Notification.permission='denied';window.dispatchEvent(new Event('focus'));});await page.getByRole('button',{name:'Voorkeuren opslaan'}).click();await page.getByText(/Notificaties zijn geblokkeerd/).waitFor();assert.ok(await page.getByRole('button',{name:'Notificaties inschakelen',exact:true}).isDisabled());assert.equal(await page.evaluate(()=>window.pushFixture.requests),0);
